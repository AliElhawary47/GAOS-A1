...
def run_reengage_check(gmail, cfg):
    ...
    for i, row in enumerate(rows):
        ...
        last_str    = str(row.get("Last Contact", "")).strip()
        re_engaged  = str(row.get("Re-Engaged",   "")).strip()
        notes       = str(row.get("Notes",        "")).strip()

        status = str(row.get("Status", "active")).strip().lower()
        if not email or not name or re_engaged:
            continue
        ...
        last_contact = core.parse_date(last_str)
        if not last_contact:
            continue
        ...
        if core.gmail_send(gmail, email, cfg["gmail"]["watch_inbox"],
            ...
            core.sheets_update_cell(sheet_id, tab, i + 2, "Re-Engaged", core.timestamp())
            sent += 1
